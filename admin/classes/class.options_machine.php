...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case "info":
					$info_text = $value['std'];
					$info_id = ($value['id'] != '') ? 'id="'.$value['id'].'"' : '' ;
					$output .= '<div class="of-info" '.$info_id.'>'.$info_text.'</div>';
				break;
				...
				//display a single image
				case "button":
					$output .= '<button class="button" type="button" id="'.$value['id'].'">'.$value['name'].'</button>';
				break;
				...
				case 'extended_slider':
                    ...
                    if ($count < 2) {
                        $oldorder = 1;
                        $order = 1;
                        $output .= Options_Machine::optionsframework_extended_slider_function($value['id'],$value['std'],$oldorder,$order);
                    } else {
                        $i = 0;
                        foreach ($slides as $slide) {
                            $oldorder = $slide['order'];
                    ...
                    $output .= '</ul>';
                    $output .= '<a href="#" class="button extended_slide_add_button">Add New Slide</a></div>';
                ...
                case 'text_slider':
                    ...
                    if ($count < 2) {
                        $oldorder = 1;
                        $order = 1;
                        $output .= Options_Machine::optionsframework_text_slider_function($value['id'],$value['std'],$oldorder,$order);
                    } else {
                        $i = 0;
                        foreach ($slides as $slide) {
                            $oldorder = $slide['order'];
                    ...
                    $output .= '</ul>';
                    $output .= '<a href="#" class="button text_slide_add_button">Add New Slide</a></div>';
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					$output .= '<textarea id="export_data" rows="8">' . bin2hex(json_encode( $data )) . '</textarea>'."\n";
	...
	/**
     * Drag and drop extended slides manager
     *
     * @uses get_option()
     *
     * @access public
     ...
     * @return string
     */
    public static function optionsframework_extended_slider_function($id,$std,$oldorder,$order){
        ...
        //initialize all vars
        $slidevars = array('title','url','text_heading','text_highlighted', 'text_normal', 'button_name', 'button_url', 'text_color');
        ...
        $slider .= '<label>Text Heading</label>';
        $slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][text_heading]" id="'. $id .'_'.$order .'_slide_text_heading" value="'. $val['text_heading'] .'" />';

        $slider .= '<label>Text Highlighted</label>';
        $slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][text_highlighted]" id="'. $id .'_'.$order .'_slide_text_highlighted" value="'. $val['text_highlighted'] .'" />';

        $slider .= '<label>Text Normal</label>';
        $slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][text_normal]" id="'. $id .'_'.$order .'_slide_text_normal" value="'. $val['text_normal'] .'" />';

        $slider .= '<label>Text Color</label>';
        $slider .= '<select style="width:320px;" class="of-input" name="'. $id .'['.$order.'][text_color]" id="'. $id .'_'.$order .'_slide_text_color">';
        $slider .= '<option id="light" value="Light" ' . selected($val['text_color'], "Light", false) . ' />Light</option>';
        $slider .= '<option id="dark" value="Dark" ' . selected($val['text_color'], "Dark", false) . ' />Dark</option>';
        $slider .= '</select>';

        $slider .= '<label>Button Name</label>';
        $slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][button_name]" id="'. $id .'_'.$order .'_slide_button_name" value="'. $val['button_name'] .'" />';

        $slider .= '<label>Button URL</label>';
        $slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][button_url]" id="'. $id .'_'.$order .'_slide_button_url" value="'. $val['button_url'] .'" />';
     ...
     * @return string
     */
    public static function optionsframework_text_slider_function($id,$std,$oldorder,$order){
        ...
        $slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][text]" id="'. $id .'_'.$order .'_slide_text" value="'. $val['text'] .'" />';
